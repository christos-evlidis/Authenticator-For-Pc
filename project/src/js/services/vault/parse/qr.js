import { vaultRecordSanitizeName } from "@/js/services/vault/record/sanitize/name.js";
import { vaultRecordSanitizeSecret } from "@/js/services/vault/record/sanitize/secret.js";
import { DATA_HOTP_COUNTER_DEFAULT, DATA_OTP_ALGORITHM_DEFAULT, DATA_OTP_DIGITS, DATA_OTP_PERIOD, DATA_OTP_TYPE_HOTP } from "@/js/const/const.otp.js";

/** Parses an otpauth URI into a normalized account object. */
function vaultParseQr(uri) {
  try {
    ...
    const secret = vaultRecordSanitizeSecret(url.searchParams.get("secret"));
    ...
    const issuer = issuerParam
      ...
      : pathIssuer;
    const { name, email } = vaultRecordSanitizeName(issuer, label);
    const account = {
      name,
      secret,
      type,
